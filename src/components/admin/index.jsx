  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <Bell size={18} />
        {unread.length > 0 && (
          <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-destructive text-primary-foreground text-[10px] font-bold flex items-center justify-center leading-none">
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-border">
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <Link
                  key={n.id}
                  to={typePath(n.type)} onClick={() => { window.scrollTo({ top: 0, behavior: "smooth" }); markRead(n.id); setOpen(false); }}
                  className={cn(
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {navItems.map(({ label, icon: Icon, path }) => {
            ...
            return (
              <Link
                key={path}
                to={path} onClick={() => { window.scrollTo({ top: 0, behavior: "smooth" }); setOpen(false); }}
                className={cn(
