...
const NavigationMenuTrigger = ({ className, children, onClick }) => (
  <button
    className={cn(
      "group inline-flex h-10 w-max items-center justify-center rounded-md bg-background px-4 py-2 text-sm font-medium transition-colors hover:bg-accent-hover hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none disabled:pointer-events-none disabled:opacity-50",
      className
    )}
    onClick={onClick}
  >
...
const NavigationMenuLink = ({ href, children }) => (
  <a href={href} className="block px-3 py-1 text-sm hover:bg-accent-hover rounded">{children}</a>
);
