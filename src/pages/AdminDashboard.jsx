...
const modules = [
  ...
  { label: "Service Requests", icon: MessageSquare, path: "/admin/service-requests", key: "serviceRequests", color: "text-indigo-500", bg: "bg-indigo-500/10" },
];
    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5">
        {modules.map(({ label, icon: Icon, path, key, color, bg }) => (
          <Link
            key={key}
            to={path} onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
      ...
      {admin.sitePhotos?.length > 0 && (
        <div className="mt-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/photo" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="text-xs text-primary hover:underline">Manage →</Link>
          </div>
