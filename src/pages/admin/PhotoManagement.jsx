            ...
            <div key={photo.id} className="group relative rounded-xl overflow-hidden border border-border aspect-video bg-muted">
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                <button
                  ...
                  className="w-9 h-9 rounded-full bg-destructive/90 flex items-center justify-center text-primary-foreground hover:bg-destructive transition-colors"
                >
                  <Trash2 size={15} />
                </button>
              </div>
              <p className="absolute bottom-0 left-0 right-0 px-2 py-1 text-xs text-primary-foreground bg-black/40 truncate opacity-0 group-hover:opacity-100 transition-opacity">
                {photo.name}
              </p>
            </div>
          ))}
